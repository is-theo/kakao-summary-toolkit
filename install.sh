...
# Download helper scripts (list-chats.sh, extract.sh)
SCRIPTS_DIR="$INSTALL_DIR/scripts"
mkdir -p "$SCRIPTS_DIR"
SCRIPTS_BASE_URL="https://raw.githubusercontent.com/${REPO_USER}/${REPO_NAME}/main/scripts"

for script in list-chats.sh extract.sh; do
  curl -fsSL "$SCRIPTS_BASE_URL/$script" -o "$SCRIPTS_DIR/$script"
  chmod +x "$SCRIPTS_DIR/$script"
done
print_success "Helper 스크립트 설치됨 → $SCRIPTS_DIR/"
...
# Use SQL JOIN to get real chat names (kakaocli chats shows '(unknown)' for groups)
"$BINARY_PATH" query \
  "SELECT cr.chatId,
...
for row in rows:
    ...
    if last_at:
        ...
        dt = '?'
    print(f'  • {name} (멤버 {members:,}명, 안읽음 {unread:,}개) — 최근: {dt}')
"
...
printf "  ${BOLD}# 채팅방 목록 (이름 + 안읽음 + 최근 시각)${NC}\n"
printf "  ~/Applications/kakaocli/scripts/list-chats.sh\n\n"
printf "  ${BOLD}# 특정 채팅방 메시지 추출 (어제 24시간치)${NC}\n"
printf "  ~/Applications/kakaocli/scripts/extract.sh <chat_id>\n\n"
printf "  ${BOLD}# 직접 명령어 (고급)${NC}\n"
